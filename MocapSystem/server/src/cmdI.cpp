...
template <typename I>
void CLIModule<I>::setState(CLI_STATE newState) {
...
void CLIModule<I>::runModule() {
    ...
    string imgSrcIn = "";
    calib->add_option("--imgSrcIn", imgSrcIn, "Img folder or 'shot' to take images");
    string imgSrcEx = "";
    calib->add_option("--imgSrcEx", imgSrcEx, "Img folder or 'shot' to take images");
    bool startCalibCalc = false;
    calib->add_option("--startCalib", startCalibCalc, "Start ex/in calib calc");
    string calibMode = "";
    calib->add_option("--mode", tion("--imgSrc", imgSrc, "Img folder");
    bool startCalibCalc = false;
    calib->add_calibMode, "Change calib mode (ex / in)");
    int row = 0, col = 0;
    ...
    while (this->moduleState == CLI_RUNNING) {
        ...
        try {
                ...
                if (controlled_capture_cmd->count("--manual") > 0) {
                    ...
                    while(true) {
                        ...
                        if (input == "") { // User pressed 'Enter'
                            payload.info.imgIdx = idx;
                            this->outNoti.push(payload);
                            idx++;
            ...
            if (dontSendMore == false) this->outNoti.push(payload);
